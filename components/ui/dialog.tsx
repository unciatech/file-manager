...
const dialogContentVariants = cva(
  'flex flex-col fixed outline-0 z-50 border border-border bg-background text-foreground shadow-lg shadow-black/5 duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
  {
    variants: {
      variant: {
        default:
          'left-[50%] top-[50%] w-full max-w-[calc(100%-16px)] translate-x-[-50%] translate-y-[-50%] p-6 max-h-[90dvh] rounded-2xl data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:max-w-lg',
        fullscreen:
          'inset-2 rounded-2xl data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:inset-5',
        command:
